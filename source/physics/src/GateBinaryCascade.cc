...
GateBinaryCascade::GateBinaryCascade() : 
G4VIntraNuclearTransportModel("modif g494 Binary Cascade")
{
...
GateBinaryCascade::GateBinaryCascade(const GateBinaryCascade& )
...
//----------------------------------------------------------------------------
G4HadFinalState * GateBinaryCascade::ApplyYourself(const G4HadProjectile & aTrack,
//----------------------------------------------------------------------------
							G4Nucleus & aNucleus)
{
...
//----------------------------------------------------------------------------
G4ReactionProductVector * GateBinaryCascade::Propagate(
//----------------------------------------------------------------------------
		G4KineticTrackVector * secondaries, G4V3DNucleus * nucleus)
{
  ...
  debug.dump();
   ...
   G4cout << "GateBinaryCascade Propagate starting -------------------------------------------------------" <<G4endl;
#endif
...
// No more collisions: absorb, capture and propagate the secondaries out of the nucleus
  ...
  if ( theSecondaryList.size() > 0 )
  {
      ...
      G4cerr << "GateBinaryCascade: Warning, have active particles at end" << G4endl;
#endif
...
//   {                                                                    // closed by Uzhi
  ...
  {                                                                       // Uzhi
	 ...
	 /*if (theDeExcitation)                // pre-compound
	 {
	  ...
	  if (theExcitationHandler)    // de-excitation
	 {
...
//----------------------------------------------------------------------------
G4double GateBinaryCascade::GetExcitationEnergy()
//----------------------------------------------------------------------------
{
...
// get A and Z for the residual nucleus
  ...
  {
     #ifdef debug_G4BinaryCascade
     G4cout << "GateBinaryCascade::GetExcitationEnergy(): Warning - invalid nucleus (A,Z)=("
...
//----------------------------------------------------------------------------
void GateBinaryCascade::BuildTargetList()
//----------------------------------------------------------------------------
{
...
#ifdef debug_BIC_BuildTargetList
     G4cout << "GateBinaryCascade::BuildTargetList():  nucleus (A,Z)=("
...
//----------------------------------------------------------------------------
void  GateBinaryCascade::FindCollisions(G4KineticTrackVector * secondaries)
//----------------------------------------------------------------------------
{
     ...
     i != secondaries->end(); ++i)
  {
    ...
    {
      G4cout << "GateBinaryCascade::FindCollisions(): negative m2:" << (*i)->GetTrackingMomentum().mag2() << G4endl;
    }
#endif
...
//----------------------------------------------------------------------------
void  GateBinaryCascade::FindDecayCollision(G4KineticTrack * secondary)
//----------------------------------------------------------------------------
{
    if ( secondary->GetTrackingMomentum().mag2() < -1.*eV )
    {
      G4cout << "GateBinaryCascade::FindDecayCollision(): negative m2:" << secondary->GetTrackingMomentum().mag2() << G4endl;
    } 
    const std::vector<G4CollisionInitialState *> & aCandList
...
//----------------------------------------------------------------------------
void  GateBinaryCascade::FindLateParticleCollision(G4KineticTrack * secondary)
//----------------------------------------------------------------------------
{
    G4RKPropagation * RKprop=(G4RKPropagation *)thePropagator;
    if ( secondary->GetTrackingMomentum().mag2() < -1.*eV )
...
//----------------------------------------------------------------------------
G4bool GateBinaryCascade::ApplyCollision(G4CollisionInitialState * collision)
//----------------------------------------------------------------------------
{
  G4ping debug("debug_ApplyCollision");
#ifdef debug_BIC_ApplyCollision
  G4cerr << "GateBinaryCascade::ApplyCollision start"<<G4endl;
  theCollisionMgr->Print();
#endif
...
//****************************************  
  ...
  if(!products || products->size()==0 || !CheckPauliPrinciple(products))
  {
     ...
     G4cerr << "GateBinaryCascade::ApplyCollision blocked"<<G4endl;
   #endif
  ...
  if ( (finalBaryon-initialBaryon-lateBaryon) != 0 || (finalCharge-initialCharge-lateCharge) != 0 ) 
     {
        G4cout << "GateBinaryCascade: Error in Balancing: " << G4endl;
...
//----------------------------------------------------------------------------
G4bool GateBinaryCascade::Absorb()
//----------------------------------------------------------------------------
{
...
//----------------------------------------------------------------------------
G4bool GateBinaryCascade::Capture(G4bool verbose)
//----------------------------------------------------------------------------
{
...
//----------------------------------------------------------------------------
G4bool GateBinaryCascade::CheckPauliPrinciple(G4KineticTrackVector * products)
//----------------------------------------------------------------------------
{
  G4int A = the3DNucleus->GetMassNumber();
  G4int Z = the3DNucleus->GetCharge();
...
//----------------------------------------------------------------------------
void GateBinaryCascade::StepParticlesOut()
//----------------------------------------------------------------------------
{
  G4int counter=0;
  G4int countreset=0;
...
//    G4cerr << "CaptureCount = "<<counter<<" "<<nsec<<" "<<minTimeStep<<" "<<1*ns<<G4endl;
    ...
    {
#ifdef debug_G4BinaryCascade
       G4cerr << "GateBinaryCascade.cc: Warning - aborting looping particle(s)" << G4endl;
#endif
...
//----------------------------------------------------------------------------
void GateBinaryCascade::CorrectFinalPandE()
//----------------------------------------------------------------------------
//
...
//  G4double m10 = massInNucleus; //pNucleus.mag();
  ...
  {
       #ifdef debug_BIC_CorrectFinalPandE
	G4cout << "GateBinaryCascade::CorrectFinalPandE() : error! " << G4endl;
...
//----------------------------------------------------------------------------
void GateBinaryCascade::UpdateTracksAndCollisions(
//----------------------------------------------------------------------------
			G4KineticTrackVector * oldSecondaries,
...
//----------------------------------------------------------------------------
G4bool GateBinaryCascade::DoTimeStep(G4double theTimeStep)
//----------------------------------------------------------------------------
{
  ...
  G4cerr <<"GateBinaryCascade::DoTimeStep: enter step="<< theTimeStep 
...
//----------------------------------------------------------------------------
G4KineticTrackVector* GateBinaryCascade::CorrectBarionsOnBoundary(
...
G4LorentzVector GateBinaryCascade::GetFinal4Momentum()
//----------------------------------------------------------------------------
{
...
#  ifdef debug_BIC_Final4Momentum
     G4cerr << G4endl;
     G4cerr << "GateBinaryCascade::GetFinal4Momentum - Fatal"<<G4endl;
     G4KineticTrackVector::iterator i;
...
#     ifdef debug_BIC_FinalNucleusMomentum
	G4cerr << "GateBinaryCascade::GetFinalNucleusMomentum - Fatal"<<G4endl;
...
//----------------------------------------------------------------------------
G4ReactionProductVector * GateBinaryCascade::Propagate1H1(
//----------------------------------------------------------------------------
		G4KineticTrackVector * secondaries, G4V3DNucleus * nucleus)
{
    G4ReactionProductVector * products = new G4ReactionProductVector;
...
//----------------------------------------------------------------------------
G4ThreeVector GateBinaryCascade::GetSpherePoint(
...
void GateBinaryCascade::ClearAndDestroy(G4KineticTrackVector * ktv)
//----------------------------------------------------------------------------
{
  std::vector<G4KineticTrack *>::iterator i;
...
//----------------------------------------------------------------------------
void GateBinaryCascade::ClearAndDestroy(G4ReactionProductVector * rpv)
//----------------------------------------------------------------------------
{
...
//----------------------------------------------------------------------------
void GateBinaryCascade::PrintKTVector(G4KineticTrackVector * ktv, std::string comment)
//----------------------------------------------------------------------------
{
...
//----------------------------------------------------------------------------
void GateBinaryCascade::PrintKTVector(G4KineticTrack * kt, std::string comment)
//----------------------------------------------------------------------------
{
  if (comment.size() > 0 ) G4cout << comment << G4endl;
    G4cout << ", id: " << kt << G4endl;
...
//----------------------------------------------------------------------------
G4bool GateBinaryCascade::CheckDecay(G4KineticTrackVector * products)
//----------------------------------------------------------------------------
{
  G4int A = the3DNucleus->GetMassNumber();
  G4int Z = the3DNucleus->GetCharge();
...
//----------------------------------------------------------------------------
G4double GateBinaryCascade::GetIonMass(G4int Z, G4int A)
//----------------------------------------------------------------------------
{
   G4double mass(0);
   if ( Z > 0 && A >= Z ) 
...
void GateBinaryCascade::PrintWelcomeMessage()
{
  G4cout <<"Thank you for using GateBinaryCascade. "<<G4endl;
}

//----------------------------------------------------------------------------
void GateBinaryCascade::DebugApplyCollision(G4CollisionInitialState * collision, 
...
//----------------------------------------------------------------------------
void GateBinaryCascade::DebugEpConservation(const G4HadProjectile & aTrack,
